...
class MonkeyPatchTest(unittest.TestCase):
    ...
    def test_remove_shm_empty_pattern_warning(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            warning_messages = [str(warning.message) for warning in w]
            self.assertTrue(any("empty pattern" in msg.lower() for msg in warning_messages))
    ...
    def test_remove_shm_no_warnings_when_disabled(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            warning_messages = [str(warning.message) for warning in w]
            self.assertFalse(any("empty pattern" in msg.lower() for msg in warning_messages))

    @patch('shmlock.shmlock_monkey_patch.resource_tracker')
    def test_remove_shm_pattern_filtering(self, mock_resource_tracker: MagicMock) -> None:
    ...
    @patch('shmlock.shmlock_monkey_patch.resource_tracker')
    def test_remove_shm_empty_pattern_cleanup_removal(self, mock_resource_tracker: MagicMock) -> None:
