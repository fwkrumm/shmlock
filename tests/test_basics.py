...
class BasicsTest(unittest.TestCase):
    ...
    def test_lock_release(self):
        ...
        with self.assertRaises(
            FileNotFoundError,
            msg=f"shm with name {shm_name} could not be "
    ...
    def test_logger(self):
        ...
        with self.assertLogs(level="INFO", logger=logger) as assert_log:
            ...
            self.assertEqual(
                assert_log.output, ["INFO:test_logger:base logger test info"]
            )
        with self.assertLogs(level="DEBUG", logger=logger) as assert_log:
            lock.debug("base logger test debug")
            self.assertEqual(
                assert_log.output, ["DEBUG:test_logger:base logger test debug"]
            )
        ...
        with self.assertLogs(level="ERROR", logger=logger) as assert_log:
            ...
            self.assertEqual(
                assert_log.output, ["ERROR:test_logger:base logger test error"]
            )
        with self.assertLogs(level="CRITICAL", logger=logger) as assert_log:
            lock.critical("base logger test critical")
            self.assertEqual(
                assert_log.output, ["CRITICAL:test_logger:base logger test critical"]
            )
        with self.assertLogs(level="ERROR", logger=logger) as assert_log:
    ...
    def test_file_logging(self):
        ...
        with tempfile.NamedTemporaryFile(
            prefix="shmlock_test_log_", suffix=".log", delete=False
        ) as temp_file:
