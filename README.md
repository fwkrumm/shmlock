...
| 4.0.0                      | Handle OSError if event handle gets invalid on Windows. Remove automatic initialization of multiprocessing Event, if user did not specify Event a mock event will be used which uses simply a time.sleep for timeout operations |
